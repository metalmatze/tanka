...
// Get retrieves a single Kubernetes object from the API
...
// GetByLabels retrieves all objects matched by the given labels from the API
func (k Kubectl) GetByLabels(namespace string, labels map[string]interface{}) ([]Manifest, error) {
	lArgs := make([]string, 0, len(labels))
	for k, v := range labels {
		lArgs = append(lArgs, fmt.Sprint("-l=%s=%s", k, v))
	}
